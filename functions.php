...
function p3_patrimonio_ensure_table() {
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") !== $table_name) {
        $sql = "CREATE TABLE $table_name (
            ...
            assigned_partner_name varchar(100) DEFAULT '',
            message text,
...
function p3_patrimonio_scripts() {
    ...
    wp_enqueue_style('p3-theme-style', get_stylesheet_uri(), array(), '1.2.1');
    ...
    if ($results) {
        foreach ($results as $row) {
            $formatted[] = array(
                ...
                'active'              => (isset($row->active) && intval($row->active) === 0) || ($row->status === 'Inativo') ? false : true,
...
function p3_patrimonio_update_lead($request) {
    ...
    // Se a chamada POST contiver action = delete, executa a exclusão diretamente
    if (isset($params['action']) && $params['action'] === 'delete') {
        return p3_patrimonio_delete_lead($request);
    }
    ...
    if (isset($params['assignedTo'])) {
        $data_to_update['assigned_to'] = sanitize_email($params['assignedTo']);
    }
    if (isset($params['assignedPartnerName'])) {
        $data_to_update['assigned_partner_name'] = sanitize_text_field($params['assignedPartnerName']);
    }
    if (!empty($data_to_update)) {
        if (is_numeric($raw_id)) {
            $wpdb->update($table, $data_to_update, array('id' => intval($raw_id)));
        } else {
            // Tenta localizar por id numérico
            $num_id = intval(preg_replace('/\D/', '', $raw_id) ?: 0);
            if ($num_id > 0) {
                $wpdb->update($table, $data_to_update, array('id' => $num_id));
            }
        }
    }
...
function p3_patrimonio_delete_lead($request) {
    ...
    p3_patrimonio_ensure_table();
    $table = $wpdb->prefix . 'p3_leads';
    $raw_id = is_array($request) ? ($request['id'] ?? '') : ($request['id'] ?? '');

    $num_id = is_numeric($raw_id) ? intval($raw_id) : intval(preg_replace('/\D/', '', $raw_id));
    $updated = false;
    if ($num_id > 0) {
        $updated = $wpdb->update(
            $table,
    ...
    return new WP_REST_Response(array(
        'success'     => true,
        'message'     => 'Lead marcado como inativo no banco de dados.',
        'inactivated' => ($updated !== false),
        'deleted_id'  => $raw_id
    ), 200);
}
...
function p3_ajax_delete_lead() {
    ...
    wp_send_json_success(array('inactivated' => true, 'id' => $raw_id, 'status' => 'Inativo'));
}
...
function p3_patrimonio_handle_lead($request) {
    ...
    if (!empty($email)) {
        ...
        // Definição dos 3 sócios consultores para distribuição uniforme
        $partners = array(
            array('email' => 'william@3ppatrimonio.com.br', 'name' => 'William Lourenço', 'phone' => '5511980846341'),
            array('email' => 'carlos@3ppatrimonio.com.br',  'name' => 'Carlos Yoshimori', 'phone' => '5511983030672'),
            array('email' => 'joao@3ppatrimonio.com.br',    'name' => 'João Silva',       'phone' => '5511996876748')
        );

        // Verificação de lead existente por e-mail: atualiza a simulação no CRM sem bloquear
        $existing_lead = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM $table WHERE LOWER(TRIM(email)) = LOWER(TRIM(%s)) LIMIT 1",
            $email
        ));
        ...
        if ($existing_lead) {
            ...
            // Se for nova simulação no formulário da página, atualiza os dados no CRM e distribui ao sócio
            $assigned_to = !empty($existing_lead->assigned_to) ? $existing_lead->assigned_to : '';
            $assigned_partner_name = !empty($existing_lead->assigned_partner_name) ? $existing_lead->assigned_partner_name : '';
            if (empty($assigned_to)) {
                $counts = array();
                foreach ($partners as $p) {
                    $c = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table WHERE assigned_to = %s", $p['email']));
                    $counts[$p['email']] = intval($c);
                }
                $chosen = $partners[0];
                $min_count = $counts[$chosen['email']];
                foreach ($partners as $p) {
                    if ($counts[$p['email']] < $min_count) {
                        $min_count = $counts[$p['email']];
                        $chosen = $p;
                    }
                }
                $assigned_to = $chosen['email'];
                $assigned_partner_name = $chosen['name'];
            }
            ...
            $update_data = array(
                ...
                'notes'                 => 'Nova simulação enviada em ' . current_time('d/m/Y H:i')
            );
            $wpdb->update($table, $update_data, array('id' => $existing_lead->id));

            return new WP_REST_Response(array(
                'success'             => true,
                'message'             => 'Simulação atualizada no CRM com sucesso!',
                'leadId'              => (string) $existing_lead->id,
                'assignedTo'          => $assigned_to,
                'assignedPartnerName' => $assigned_partner_name
            ), 200);
        }
    }

    // 1. Distribuição igualitária e equilibrada entre os 3 sócios da 3P Patrimônio
    // Sócios: William Lourenço, Carlos Yoshimori e João Silva
    $partners = array(
        array('email' => 'william@3ppatrimonio.com.br', 'name' => 'William Lourenço', 'phone' => '5511980846341'),
        array('email' => 'carlos@3ppatrimonio.com.br',  'name' => 'Carlos Yoshimori', 'phone' => '5511983030672'),
        array('email' => 'joao@3ppatrimonio.com.br',    'name' => 'João Silva',       'phone' => '5511996876748')
    ...
    $assigned_to = sanitize_email($params['assignedTo'] ?? ($params['assigned_to'] ?? ''));
    $assigned_partner_name = sanitize_text_field($params['assignedPartnerName'] ?? ($params['assigned_partner_name'] ?? ''));

    // Se o sócio não foi pré-atribuído, calcula com base na menor carga do banco (distribuição estritamente igualitária)
    if (empty($assigned_to)) {
        $counts = array();
        foreach ($partners as $p) {
            $c = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table WHERE assigned_to = %s", $p['email']));
        ...
        // Escolhe o sócio com menor número de leads acumulados
        $chosen = $partners[0];
        $min_count = $counts[$chosen['email']];
        foreach ($partners as $p) {
            if ($counts[$p['email']] < $min_count) {
        ...
        $assigned_to = $chosen['email'];
        $assigned_partner_name = $chosen['name'];
    } elseif (empty($assigned_partner_name)) {
        foreach ($partners as $p) {
            if ($p['email'] === $assigned_to) {
                $assigned_partner_name = $p['name'];
                break;
            }
        }
        if (empty($assigned_partner_name)) {
            $assigned_partner_name = 'Sócio 3P';
        }
    }
    ...
    if ($inserted === false) {
        ...
        $fallback_data = array(
            ...
            'credit_amount'       => $credit,
            'monthly_installment' => $parcel,
            'message'             => "[Origem: {$source}] " . ($time_frame ? "[Prazo: {$time_frame}] " : "") . "[Sócio: {$assigned_partner_name}] " . $msg,
            'status'              => 'Novo'
        );
    ...
    // Redundância 2: Notificação por E-mail para os Sócios / Consultores (Carlos Yoshimori e William Lourenço)
    if (function_exists('wp_mail')) {
        $admin_mail = get_option('admin_email');
        $destinatarios = array_unique(array_filter(array(
        ...
        // NOTIFICAÇÃO OBRIGATÓRIA: Qualquer lead que chega vai para o WhatsApp Central (11) 99687-6748, independente do sócio
        $central_phone = '5511996876748';
        $central_msg = rawurlencode("🔔 *NOVO LEAD 3P PATRIMÔNIO*\n\nChegou um novo lead no site!\n👤 *Nome:* {$name}\n📱 *WhatsApp:* {$whatsapp}\n✉️ *E-mail:* " . ($email ?: "Não informado") . "\n🎯 *Objetivo:* {$obj}\n💰 *Crédito:* {$credit}\n💳 *Parcela:* {$parcel}\n" . ($time_frame ? "⏱️ *Prazo:* {$time_frame}\n" : "") . ($bidding ? "🏷️ *Lance:* {$bidding}\n" : "") . "📋 *Origem:* {$source}\n👉 *Falar com cliente:* https://wa.me/55{$whats_clean}\n\n_Notificação enviada para o WhatsApp (11) 99687-6748._");
        $link_whatsapp_central = "https://wa.me/{$central_phone}?text={$central_msg}";

        // Link para notificar o sócio Carlos Yoshimori diretamente no WhatsApp dele: (11) 98303-0672
        $carlos_phone = '5511983030672';
        $carlos_msg = rawurlencode("🔔 *NOVO LEAD 3P PATRIMÔNIO*\n\nOlá Carlos Yoshimori! Novo lead atribuído:\nCliente: {$name}\nWhatsApp: {$whatsapp}\nObjetivo: {$obj}\nCrédito: {$credit}\nParcela: {$parcel}\nFalar com cliente: https://wa.me/55{$whats_clean}");
        $link_whatsapp_carlos = "https://wa.me/{$carlos_phone}?text={$carlos_msg}";

        // Link para notificar o sócio William Lourenço diretamente no WhatsApp dele: (11) 98084-6341
        $william_phone = '5511980846341';
        $william_msg = rawurlencode("🔔 *NOVO LEAD 3P PATRIMÔNIO*\n\nOlá William Lourenço! Novo lead atribuído:\nCliente: {$name}\nWhatsApp: {$whatsapp}\nObjetivo: {$obj}\nCrédito: {$credit}\nParcela: {$parcel}\nFalar com cliente: https://wa.me/55{$whats_clean}");
        $link_whatsapp_william = "https://wa.me/{$william_phone}?text={$william_msg}";
        ...
        $corpo .= "NOME: {$name}\n";
        $corpo .= "WHATSAPP DO CLIENTE: {$whatsapp}\n";
        $corpo .= "LINK WHATSAPP DO CLIENTE: https://wa.me/55{$whats_clean}\n\n";
        $corpo .= "🚨 NOTIFICAR NO WHATSAPP (11 99687-6748) - QUALQUER LEAD:\n{$link_whatsapp_central}\n\n";
        $corpo .= "AVISAR SÓCIO CARLOS NO WHATSAPP (11 98303-0672):\n{$link_whatsapp_carlos}\n\n";
        $corpo .= "AVISAR SÓCIO WILLIAM NO WHATSAPP (11 98084-6341):\n{$link_whatsapp_william}\n\n";
    ...
    return new WP_REST_Response(array(
        ...
        'assignedTo'          => $assigned_to,
        'assignedPartnerName' => $assigned_partner_name,
        'whatsappAlertUrl'    => $link_whatsapp_central,
        'notificationPhone'   => '5511996876748'
    ), 200);
}
...
function p3_theme_handle_export_leads() {
    ...
    if ($action === 'delete_lead') {
        ...
        check_admin_referer('p3_delete_lead_action');
        ...
        wp_safe_redirect(admin_url('admin.php?page=p3-leads-manager&inactivated=1'));
        exit;
    }
    ...
    if ($action === 'restore_lead') {
        ...
        check_admin_referer('p3_restore_lead_action');
        $lead_id = intval($_GET['lead_id'] ?? 0);
        if ($lead_id > 0) {
            global $wpdb;
            $table_name = $wpdb->prefix . 'p3_leads';
        ...
        wp_safe_redirect(admin_url('admin.php?page=p3-leads-manager&restored=1'));
        exit;
    }
}
...
if (!function_exists('p3_register_admin_menu')) {
    ...
    function p3_patrimonio_render_admin_crm() {
        ...
        echo '<th style="font-weight: 700; width: 80px; text-align: center;">Ações</th>';
        echo '</tr></thead>';
        echo '<tbody>';
        ...
        if ($total > 0) {
            foreach ($leads as $l) {
                ...
                $is_inactive = ($l->status === 'Inativo' || (isset($l->active) && intval($l->active) === 0));
                $delete_url = wp_nonce_url(admin_url('admin.php?page=p3-leads-manager&action=delete_lead&lead_id=' . $l->id), 'p3_delete_lead_action');
                $restore_url = wp_nonce_url(admin_url('admin.php?page=p3-leads-manager&action=restore_lead&lead_id=' . $l->id), 'p3_restore_lead_action');

                $badge_status = $is_inactive
                    ? '<span style="background: #fee2e2; color: #b91c1c; border: 1px solid #fca5a5; padding: 3px 8px; border-radius: 9999px; font-weight: 700; font-size: 10px;">Inativo no Banco</span>'
                    : '<span style="background: #f1f5f9; color: #334155; padding: 3px 8px; border-radius: 9999px; font-weight: 700; font-size: 11px;">' . esc_html($l->status) . '</span>';

                $action_btn = $is_inactive
                    ? '<a href="' . esc_url($restore_url) . '" style="color: #059669; font-size: 11px; font-weight: 700; text-decoration: none; padding: 4px 10px; border-radius: 6px; background: #d1fae5; border: 1px solid #a7f3d0;">Reativar</a>'
                    : '<a href="' . esc_url($delete_url) . '" onclick="return confirm(\'Deseja marcar este lead como Inativo no banco de dados?\');" style="color: #ef4444; font-size: 11px; font-weight: 600; text-decoration: none; padding: 4px 8px; border-radius: 6px; background: #fee2e2;">Inativar</a>';
                ...
                echo '<td>' . $badge_status . '</td>';
                echo '<td style="text-align: center;">' . $action_btn . '</td>';
                echo '</tr>';
            }
        } else {
            echo '<tr><td colspan="9" style="text-align: center; padding: 48px; color: #64748b;">Nenhum lead capturado ainda. Assim que um visitante preencher o formulário ou solicitar o e-book, o contato aparecerá aqui instantaneamente.</td></tr>';
        }
