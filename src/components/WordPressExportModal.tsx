        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
          ...
          {activeTab === 'github' && (
            <div className="space-y-6 text-xs text-slate-300">
              ...
              {/* Problema da Foto dos Sócios */}
              <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/30 p-5 rounded-2xl space-y-4">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
                    <ImageIcon className="w-5 h-5 shrink-0" />
                    <span>Por que a foto não está aparecendo após restaurar o backup?</span>
                  </div>
                  <a
                    href="/screenshot.png"
                    download="screenshot.png"
                    className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-3.5 py-1.5 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-md"
                  >
                    <Download className="w-4 h-4" />
                    <span>Baixar Foto dos Sócios (screenshot.png)</span>
                  </a>
                </div>
                <p className="text-slate-300 leading-relaxed">
                  Ao restaurar um backup antigo na Hostinger, o WordPress recupera o estado de quando o backup foi gerado. Se o arquivo da foto dos sócios não existia naquela data dentro da pasta do tema, ele dará erro 404.
                </p>
                <div className="bg-slate-950 border border-slate-800 p-3.5 rounded-xl space-y-2">
                  <div className="font-bold text-white text-[11px] uppercase tracking-wider text-amber-400">
                    Como resolver a foto em 30 segundos no Gerenciador de Arquivos da Hostinger:
                  </div>
                  <ol className="list-decimal list-inside space-y-1.5 text-slate-300 text-[11px]">
                    <li>Baixe a foto oficial dos sócios pelo botão acima (<code className="font-mono text-amber-300">screenshot.png</code>).</li>
                    <li>No painel da Hostinger (hPanel), abra o <strong>Gerenciador de Arquivos</strong>.</li>
                    <li>Vá até a pasta: <code className="text-amber-300 font-mono bg-slate-900 px-1 py-0.5 rounded">public_html/wp-content/themes/3p-patrimonio/assets/</code></li>
                    <li>Envie (Upload) o arquivo <code className="font-mono text-white">screenshot.png</code> para dentro dessa pasta.</li>
                    <li>(Opcional) Coloque também uma cópia na raiz do tema: <code className="text-amber-300 font-mono bg-slate-900 px-1 py-0.5 rounded">public_html/wp-content/themes/3p-patrimonio/screenshot.png</code>.</li>
                  </ol>
                </div>
              </div>
