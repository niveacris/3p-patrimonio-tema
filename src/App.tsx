...
import { syncPendingVaultLeads, getLocalVaultLeads } from './utils/leadsStorage';
      ...
      if (res.ok) {
        ...
        if (data.leads && Array.isArray(data.leads)) {
          ...
          const unsyncedLeads: Lead[] = vaultItems
            .filter(v => !v.synced && !data.leads.some((l: Lead) => l.whatsapp === v.data.whatsapp))
            .map(v => ({
              id: v.id,
          ...
          setLeads([...unsyncedLeads, ...data.leads]);
        }
      }
    } catch (e) {
      console.error('Error fetching leads:', e);
      // Em caso de falha de rede, carrega os registros do cofre local
      const vaultItems = getLocalVaultLeads();
